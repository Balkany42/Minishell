...
/*   Updated: 2026/07/21 01:55:35 by megrelli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/minishell_bonus.h"

static t_token	*parse_word_token(t_cmd *cmd, t_token *tok, t_minishell *sh)
{
	char	*expanded;
	char	**parts;
	int		i;
...
static int	is_ambiguous_redirect(t_tokentype type, char *expanded)
{
	if (type == HEREDOC)
		return (0);
	if (!expanded[0])
...
static void	redir_error(t_cmd *cmd, t_token *tok, char *expanded,
		t_minishell *sh)
{
