...
/*   Updated: 2026/07/21 01:54:39 by megrelli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/minishell_bonus.h"

static t_cmd	*parse_pipeline_step(t_token **list, t_minishell *sh,
		t_cmd **head)
{
	t_cmd	*cmd;
	t_ast	*g;
...
static t_ast	*parse_and_or_step(t_ast *left, t_token **t, t_minishell *sh)
{
	t_ast_op	op;
	t_ast		*right;
	t_ast		*node;
